...
void *kmalloc(size_t size) {
    ...
    //申请超过1024字节
    if (size > 1024) {
        const u32 asize = size + sizeof(arena_t);   //总共需要多大内存
        const u32 count = div_round_up(asize, PAGE_SIZE);   //多少页
